...
# Define the kernel for benchmarking
def run_training(operators, num_layers: int, required_iterations: int, input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    if config['done'] == False:
        ...
        if list_attemps:
            ...
            for h in range(number_of_unique_operators_in_model):
                ...
                num_repeats = 1  # Number of times to repeat the measurement
                timer = benchmark.Timer(
                    stmt="run_training(operators, num_layers,required_iterations, ifmap)",  # Statement to benchmark  # Setup the function and variables
