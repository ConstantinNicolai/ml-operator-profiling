...
parser = argparse.ArgumentParser(description="Parse two filenames from the command line.")
parser.add_argument("file1", type=str, help="First filename")
parser.add_argument("file2", type=str, help="Second filename")
parser.add_argument("file3", type=str, help="name for output plots")
...
def read_prediction_file(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for i in range(0, len(lines), 5):  # Every 4 lines form a single entry
            ...
            predictions[model_input_size] = energy_mJ, energy_error  # Convert mJ to J
    return predictions
...
measurement_file = args.file1
prediction_file = args.file2
...
threshold = 165  # Now in Joules (since mJ to J conversion is done)
...
if small_indices:
    ...
    plt.tight_layout()
    plt.savefig('plots/testing/time' +args.file3 + '_small.png', format='png')
    plt.savefig('plots/testing/time'+args.file3 + '_small.pdf', format='pdf')
...
if large_indices:
    ...
    plt.tight_layout()
    plt.savefig('plots/testing/time'+args.file3 + '_large.png', format='png')
    plt.savefig('plots/testing/time'+args.file3 + '_large.pdf', format='pdf')
